  ...
  return (
    <button className="friend-card" onClick={onOpen}>
      ...
      <div className="friend-signals">
        <span>
          <MessageCircle size={14} /> <b>Talk</b>
          {friend.communication || "Add a communication note"}
        </span>
        <span>
          <Sparkles size={14} /> <b>Motivates</b>
          {friend.motivates || "Add what gives them energy"}
        </span>
        <span>
          <Zap size={14} /> <b>Be mindful</b>
          {friend.avoid || "Add what to be mindful of"}
        </span>
      </div>
      <span className="open-profile">
...
export default function FriendsView({
  ...
  const saveDraft = () => {
    ...
    setFriends((current) =>
      current.some((friend) => friend.id === saved.id)
        ...
        <header className="view-heading profile-heading">
          <div>
            <span className="eyebrow">
              <UserRound size={13} /> Friend profile
            </span>
            <h1>
              {friends.some((friend) => friend.id === draft.id)
                ? "Edit friend"
                : "Add a friend"}
            </h1>
            <p>Capture what helps you understand and care for this person.</p>
          </div>
          <button className="secondary-action" onClick={closeEditor}>
            <X size={16} /> Cancel
          </button>
        </header>
        ...
        >
          <section className="form-panel identity-panel">
            <div className="form-section-heading">
              ...
              <div>
                <h2>Identity</h2>
                <p>The essentials shown at the top of their card.</p>
              </div>
            </div>
              ...
              <label className="field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setDraft({
                      ...draft,
                      personalityType: event.target
                        .value as Friend["personalityType"],
                    })
                  }
                >
          ...
          <section className="form-panel">
            <div className="form-section-heading">
              ...
              <div>
                ...
                <p>Short notes that stay visible on the card.</p>
              </div>
            </div>
            <div className="form-grid">
              <label className="field span-two">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setDraft({ ...draft, communication: event.target.value })
                  }
                  placeholder="e.g. Be direct, but give her time to process"
                />
              </label>
              <label className="field span-two">
                <span>What motivates them</span>
                <input
                  value={draft.motivates}
                  onChange={(event) =>
                    setDraft({ ...draft, motivates: event.target.value })
                  }
                  placeholder="e.g. New possibilities and shared plans"
                />
              </label>
              <label className="field span-two">
                <span>What to be mindful of</span>
                <input
                  value={draft.avoid}
                  onChange={(event) =>
                    setDraft({ ...draft, avoid: event.target.value })
                  }
                  placeholder="e.g. Pressure and last-minute control"
                />
              </label>
            </div>
          </section>
          ...
          <section className="form-panel">
            <div className="form-section-heading">
              ...
              <div>
                ...
                <p>Context available after opening the card.</p>
              </div>
            </div>
            <div className="form-grid">
              <label className="field span-two">
                <span>Important needs</span>
                <textarea
                  value={draft.needs}
                  onChange={(event) =>
                    setDraft({ ...draft, needs: event.target.value })
                  }
                  placeholder="What helps them feel safe, understood or supported?"
                />
              </label>
              <label className="field span-two">
                ...
                <textarea
                  ...
                  placeholder="Patterns, memorable situations, things worth remembering…"
                />
              </label>
            </div>
          </section>
  ...
  if (mode === "detail" && selected) {
    return (
      <div className="friends-view profile-view">
        <button className="back-button" onClick={() => setMode("grid")}>
          <ArrowLeft size={16} /> All friends
        </button>
        <header className="friend-profile-hero">
          ...
          <div>
            ...
            <h1>{selected.name}</h1>
            <p>{selected.relationship || "Friend"}</p>
          </div>
          <span className="type-badge large">
            {selected.personalityType || "Type not set"}
          </span>
          <button className="secondary-action" onClick={beginEdit}>
        ...
        <div className="profile-grid">
          ...
          <ProfileSection
            ...
            value={selected.motivates}
          />
          <ProfileSection
            title="What to be mindful of"
            value={selected.avoid}
          />
          <ProfileSection title="Important needs" value={selected.needs} />
          <section className="profile-section wide">
            <span className="section-label">Notes & observations</span>
            <p>{selected.notes || "Nothing added yet."}</p>
          </section>
        </div>
      </div>
    );
  ...
  return (
    <div className="friends-view">
      <header className="view-heading">
        <div>
          <span className="eyebrow">
            <UsersIcon /> People map
          </span>
          <h1>Friends</h1>
          <p>Small notes that help you understand the people you care about.</p>
        </div>
      ...
      {friends.length === 0 ? (
        <section className="friends-empty">
          <span className="empty-friend-icon">
            <UserRound size={25} />
          </span>
          <h2>Your people map is empty.</h2>
          <p>
            Add someone you care about. Their card can hold the little things
            that make communication easier.
          </p>
          <button className="primary-action" onClick={beginCreate}>
            <Plus size={16} /> Add your first friend
          </button>
        </section>
      ) : (
